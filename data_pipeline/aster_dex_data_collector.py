...
class AsterDEXDataCollector:
    ...
    async def initialize(self):
        ...
        # Test connectivity and get available symbols
        try:
            ...
            if symbols:
                logger.info(f"✅ Loaded {len(symbols)} available symbols")
            else:
                logger.warning("⚠️  Could not load available symbols")
    ...
    async def get_available_symbols(self) -> List[str]:
        """Get list of available trading symbols from Aster DEX."""
        try:
            if not self.session:
            ...
            async with self.session.get(url, timeout=self.request_timeout) as response:
                if response.status == 200:
                    ...
                    logger.info(f"Retrieved {len(symbols)} available symbols from Aster DEX")
                    return symbols
                else:
                    logger.error(f"Failed to get exchange info: HTTP {response.status}")
                    return []
        except Exception as e:
