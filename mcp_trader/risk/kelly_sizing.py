...
class KellySizer:
    ...
    def __init__(self, config: KellyConfig):
        ...
        logger.info(f"📊 Fractional Kelly: {config.kelly_fraction:.1%}")
    ...
    def calculate_kelly_fraction(self,
        ...
        try:
            ...
            if win_prob < 0 or win_prob > 1:
                logger.warning(f"⚠️ Invalid win probability: {win_prob}, using default")
            ...
            if payoff_ratio <= 0:
                logger.warning(f"⚠️ Invalid payoff ratio: {payoff_ratio}, using default")
            ...
            if full_kelly <= 0:
                logger.warning("⚠️ Negative Kelly fraction - no edge detected")
                return 0.0
            ...
            logger.debug(f"📊 Kelly: Full={full_kelly:.3f}, Fractional={fractional_kelly:.3f}")
    ...
    def calculate_position_quantity(self,
        ...
        try:
            ...
            if current_price <= 0:
                logger.error("❌ Invalid price for quantity calculation")
                return 0.0
            
            quantity = position_size_usd / current_price
            
            logger.debug(f"📊 Position Quantity: {quantity:.6f} @ ${current_price:.2f}")
            
            return quantity
            
        except Exception as e:
            logger.error(f"❌ Position quantity calculation error: {e}")
            return 0.0
    ...
    def update_win_probability(self, won: bool):
        ...
        try:
            ...
            logger.debug(f"📈 Win Rate: {recent_win_rate:.1%} ({self.win_count}/{self.total_trades})")
            
        except Exception as e:
            logger.error(f"❌ Win probability update error: {e}")
    ...
    def get_estimated_win_probability(self, strategy: Optional[str] = None) -> float:
        ...
        try:
            ...
            logger.error(f"❌ Win probability estimation error: {e}")
            return self.config.default_win_prob
    ...
    def calculate_risk_of_ruin(self,
        ...
        try:
            ...
            logger.debug(f"⚠️ Risk of Ruin: {p_ruin:.2%} over {num_trades} trades")
            
            return p_ruin
            
        except Exception as e:
            logger.error(f"❌ Risk of ruin calculation error: {e}")
...
def calculate_sharpe_optimal_kelly(mean_return: float,
    ...
    try:
        ...
        logger.error(f"❌ Sharpe-optimal Kelly calculation error: {e}")
        return 0.1
